    ...
    <div id="section-role" className="w-full bg-white p-4 rounded-xl border border-slate-200 mb-5 shadow-2xs scroll-mt-24 transition-all">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          />
          <label className="text-sm font-semibold text-slate-700 flex items-center gap-1.5 cursor-pointer">
        ...
        <div className="relative mt-2" ref={containerRef}>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full text-xs text-slate-900 bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-8 py-2 focus:bg-white focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500 transition-colors shadow-2xs"
            />
              ...
              {filteredRoles.map((role) => {
                ...
                return (
                  <div
                    ...
                    className={`px-3 py-2 text-xs cursor-pointer flex items-center justify-between hover:bg-sky-50 ${isSelected ? "bg-sky-50/70 text-sky-800 font-semibold" : "text-slate-700"
                      }`}
                  >
                    <span>{role.name}</span>
